  ...
  <div data-role="page" data-url="/_display/" tabindex="0" class="ui-page ui-body-c ui-page-active" style="min-height: 550px;">
   ...
   <div class="cta-wrapper clearfix">
    ...
    if($user_ID != $session_user_ID ){
      ?>
      <div class="cta-btn-wrapper"><p class="cta-btn btn btn-6 btn-6j">Mitessen</p></div>
      <?php 
    } 
    if($user_ID == $session_user_ID){
      ?>
      <form class="icon-wrapper" method="post" action="?act=delete"><button name="delete" class="btn btn-7 btn-7d btn-icon-only icon-remove"></form>
      <?php
    }
    ?>
  </div>
...
function delete_meal(){
  require_once 'connect.php';
  $delete = mysql_query("DELETE * FROM meal_tbl WHERE meal_ID='$meal_ID'");
  //var_dump($delete);
  if(!$delete){
