...
<div id="container" class="content-wrapper">
	<div class="content clearfix">
		<div class="home-head-wrapper clearfix">
			<?php $loop = new WP_Query( array('pagename' => 'live')); ?>
			<?php while ( $loop->have_posts() ) : $loop->the_post();?>
				<?php  
				$post_status_page = get_post_status();
				if($post_status_page == live){ ?>
				<div class="now-live-wrapper">
					<h1>GLFtv jetzt LIVE ansehen!</h1> <a href="<?php the_permalink(); ?>">Hier.</a>
				</div>
				<?php } ?>
			<?php endwhile; ?>
			<?php wp_reset_postdata(); ?>
			<?php wp_reset_query(); ?>
			<div class="home-head left-section">
			...
			<div class="home-head right-section">
				<div>
					<?php $loop = new WP_Query( array('pagename' => 'live')); ?>
					<?php while ( $loop->have_posts() ) : $loop->the_post();?>
	...
	<?php $loop = new WP_Query( array('post_type' => 'startseite', 'meta_key' => 'kacheltyp', 'meta_value' => 'redaktionell', 'posts_per_page' => 15)); ?>
	<?php $loop_post_count = $loop->post_count; ?>
	<?php if($loop->have_posts() && $loop_post_count >= 5){?>
	<div class="tile-wrapper home-tile-wrapper clearfix"> <!-- bitte Inhalte kontrollieren !-->
		<div class="headline-wrapper">
			<?php $loop = new WP_Query( array('page_id' => '670')); ?>
			<?php while ( $loop->have_posts() ) : $loop->the_post();?>
				<?php 
				$video_headline = get_field('news_headline'); 
				?>
		...
		<ul class="bxslider">	
			...
			<?php while ( $loop->have_posts() ) : $loop->the_post();?>
				...
				?>
				<?php
				if ($kacheltyp == "redaktionell") {
					$headline_redaktionell = get_field('headline-redaktionell');
					$date_redaktionell = get_field('date-redaktionell');
					$img_redaktionell = get_field('img-redaktionell');
					$link_redaktionell = get_field('link-redaktionell');
					?>
					<li>
						<a href="<?php echo $link_redaktionell; ?>" target="_blank" class="tile teaser-tile redaktionell-teaser-tile" >
							<?php
							$tile_img['s'] = wp_get_attachment_image_src($img_redaktionell, 'kachel');
							$tile_img['m'] = wp_get_attachment_image_src($img_redaktionell, 'thema_m');
							?>
							<figure class="teaser-tile-img responsive" data-media="<?php echo $tile_img['m'][0]; ?>" data-media480="<?php echo $tile_img['s'][0]; ?>">
								<noscript><img src="<?php echo $tile_img['s'][0]; ?>" alt="Bild"></noscript>
							</figure>
							<div class="tile-text-layer">
								<div class="extra-info-wrapper">
									<p class="text extra-info-text date-text">
										<?php 
										$localyzedate = date_i18n("d. F Y", strtotime($date_redaktionell));
										echo $localyzedate;
										?>
									</p>
								</div>
								<h3 class="tile-title">
									...
									if(strlen($headline_redaktionell)>40) {
										echo substr($headline_redaktionell,0,40).'...';
									} else {
										echo $headline_redaktionell;
									}
									?>
								</h3>
							</div>
